...
int load_dataset(Store *stores, const char *base_path)
{
    ...
    {
        if (store_entry->d_name[0] == '.') // نادیده گرفتن "." و ".."
            continue;
        ...
        if (S_ISDIR(store_stat.st_mode)) // بررسی اینکه آیا این مسیر یک دایرکتوری است
        {
            ...
            {
                if (category_entry->d_name[0] == '.') // نادیده گرفتن "." و ".."
                    continue;
                ...
                if (S_ISDIR(category_stat.st_mode)) // بررسی اینکه آیا این مسیر یک دایرکتوری است
                {
